...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        try:
            ...
            return repo.__del__()
        await event.edit("`Successfully Force Updated to official repo!\n" "Restarting, please wait...`")

        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#UPDATE \n" "Your Fizilion was successfully updated to official repo"
            )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "`Successfully Force Updated to Official Repo!\n" "Bot is restarting... Wait for a second!`"
    )
...
async def upstream(event):
    ...
    ac_br = "dragon"
    if ac_br != FUPSTREAM_REPO_BRANCH:
        await event.edit(
            "**[UPDATER]:**\n"
    ...
    if changelog == "" and force_update is False:
        await event.edit(
            f"\n`{UPDATER_ALIAS} is`  **up-to-date**  `with`  **{FUPSTREAM_REPO_BRANCH}**\n"
        )
        return repo.__del__()
    ...
    if conf is None and force_update is False:
        ...
        return await event.respond('`do ".fota now/deploy" to update`')
